...
train_directory = "Dataset1/labels"
for root, dirs, directory in os.walk(train_directory):
    for i in range(len(directory)):
        img = cv2.imread(train_directory+"/"+directory[i],0)
        img = cv2.resize(img,(64,64), interpolation = cv2.INTER_CUBIC)
...
hist = model.fit(x = train_vol, y = train_seg, batch_size = 16, epochs = 100, validation_data =(test_vol,test_seg))
...
test = 'dataset/train/MCUCXR_0001_0.png'
...
preds = preds[0]
print(preds.shape)
cv2.imwrite('myimg.png',cv2.resize(preds*255,(512,512),interpolation = cv2.INTER_CUBIC))
cv2.imshow('orig',cv2.resize(cv2.imread(test),(400,400),interpolation = cv2.INTER_CUBIC))
cv2.imshow('mask',cv2.resize(cv2.imread('dataset/labels/MCUCXR_0001_0.png'),(400,400),interpolation = cv2.INTER_CUBIC))
cv2.imshow("ll",cv2.resize(preds,(400,400),interpolation = cv2.INTER_CUBIC))
cv2.waitKey(0)
